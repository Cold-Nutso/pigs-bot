...
const {
  Client, IntentsBitField, ChannelType, PermissionsBitField, EmbedBuilder,
...
const getActiveGame = (playerName) => {
  ...
  for (let i = 0; i < pigs.activeGames.length; i++) {
    if (pigs.activeGames[i][`${playerName}`]) {
      foundGame = pigs.activeGames[i];
      break;
    }
  }
...
const handleStats = (msg, param) => {
  ...
  if (!param) { targetUser = msg.author; } else { targetUser = getUserFromMention(param); }
  ...
  if (!targetUser) {
    ...
    if (!statSet) { statSet = pigs.addPlayer(targetUser.username); }
...
const handleRoll = (msg, activeGame, param) => {
  ...
  if (activeGame && activeGame.activePlayer === userName) {
    ...
    // Grab variables
    const userProfit = pigs.takeTurn(activeGame, userName, p); // Take the actual turn
    ...
    if (userProfit === 0) {
      ...
      userTurn = activeGame[`${userName}`].turn;
    }
...
const handleCall = (msg, activeGame) => {
  ...
  if (activeGame && activeGame.activePlayer === userName) {
    ...
    if (activeGame.activePlayer === client.user.username) { msg.reply('Oink! It\'s my turn!'); } else { msg.reply(`Oink! It's ${activeGame.activePlayer}'s turn!`); }
  } else {
...
const handleStart = (msg, param) => {
  ...
  if (!param) {
    ...
    if (opponent.username === playerName) {
      ...
      if (yourTurn) {
        currentOpponent = activeGame.waitingPlayer;
      } else {
        currentOpponent = activeGame.activePlayer;
      }
      ...
      if (opponent.username === client.user.username) { response += 'Sure, let\'s play a game!'; } else { response += `Hey ${opponent}!\n${msg.author} started a game of pigs with you!`; }
  ...
  const activeGame = getActiveGame(playerName);
  // Since turn is over, bot should go
  if (activeGame.activePlayer === client.user.username) {
    handleTurn(msg.channel, activeGame);
  }
